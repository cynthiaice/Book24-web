...
class EventCategorys extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  this.props.router.push('/login');
    ...
    axios
      .get(API_URL + "eventCategorys", config)
      .then(response => {
    ...
    //      router.push("/");
        ...
        this.setState({ eventCategorys: [] });
        for (let i = 0; i < len; i++) {
          let row = response.data.rows[i];
          this.setState(prevState => ({
            eventCategorys: [...prevState.eventCategorys, row]
          }));
        }
      })
      .catch(error => {
  ...
  createEventCategory(e) {
    e.preventDefault();
      ...
      axios
        .post(API_URL + "eventCategory", bodyParameters, config)
        .then(response => {
          console.log(response);
  ...
  render() {
    ...
    const eventCategory_data = this.state.eventCategorys.map(
      function(item, index) {
        return (
          <tr>
            <td>{index + 1}</td>
    ...
    const ModalEventCategory = ({ handleClose, show, children }) => {
    ...
    return (
      ...
      <div className="main-box">
        ...
        >
          <p>Are you sure you want to delete this eventCategory</p>
        </ModalEventCategory>
        <p className="admin-header">Ticket Type</p>
        <button
        ...
        <div className={showHideClassNameCreate}>
          <p className="admin-header">Add Event Category</p>
          {this.state.error_div && (
            <div className="error">{this.state.error}</div>
          )}
          <form onSubmit={this.createEventCategory.bind(this)}>
            ...
            <div className="input-row">
              {/* <button onClick={this.createEventCategory.bind(this)}>Create</button>{" "} */}
...
export default withRouter(EventCategorys);
